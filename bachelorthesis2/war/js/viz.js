function getMelodyArray(melodyIndex) {	
	var ngram = $("#KeyboardTimeAgeContainer .gwt-TextBox").val();
...
function switchNgramRepresentation() {
	var ngramType = $("#ngramTimeAgeContainer select").val();
	...
	$("#KeyboardTimeAgeContainer  .gwt-TextBox").val(out);
}
...
function onGwtReady() {
	...
	$("#ngramTimeAgeContainer select").change(function() {
	...
	var contd = $("#KeyboardTimeAgeContainer  .gwt-TextBox");
	...
	contd.live('keyup', function(e){
		var c = e.which;
		var ngramField = $("#KeyboardTimeAgeContainer  .gwt-TextBox");
...
var playedNote = function(pitch) {
	prevNotePiano = pitch;
	var prevNgram = $("#KeyboardTimeAgeContainer  .gwt-TextBox").val();
	...
	if (playedMelody.length > 0) {
		...
		$("#KeyboardTimeAgeContainer  .gwt-TextBox").val(newNgram);
	} else /* if (prevNgram != "") {
		var newNgram = prevNgram + ", " + pitch;
		$("#contentContainer .gwt-TextBox").val(newNgram);
	} else if (prevNgram == "") */ {
		$("#KeyboardTimeAgeContainer  .gwt-TextBox").val(pitch);
	}
	renderInput();
...
function addNoteAfter(noteIndex) {
	...
	$("#KeyboardTimeAgeContainer  .gwt-TextBox").val(new_mel_str);
...
function getMelodyArray2(melodyIndex) {	
	var ngram = $("#KeyboardTimeCountryContainer .gwt-TextBox").val();
	ngram = ngram.replace(/\s{2,}/g, ' ');
	ngram = ngram.replace(/\-{2,}/g, '-');
	ngram = ngram.replace(/^\s+|\s+$/g, '');
	var melodies = ngram.split(',');
...
var renderInput2 = function() {
	...
	for (var m = 0; m < melodies.length; m++) {
		...
		renderMel2(melodies[m], canvas[0]);
	}
...
function switchNgramRepresentation2() {
	var ngramType = $("#ngramTimeCountryContainer select").val();
	...
	out = out.substring(0, out.length - 2);
	$("#KeyboardTimeCountryContainer .gwt-TextBox").val(out);
}


var timerTimeCountry1, timerTimeCountry2;

function onGwtReady2() {
	renderInput2();
	flashPianoInit2();
	...
	$("#ngramTimeCountryContainer select").change(function() {
		switchNgramRepresentation2();
		renderInput2();
	});
	...
	var contd = $("#KeyboardTimeCountryContainer .gwt-TextBox");
	contd.keydown(function(e) {
		if (e.which == '32') {
			renderInput2();
		}
	...
	contd.live('keyup', function(e){
		var c = e.which;
		var ngramField = $("#KeyboardTimeCountryContainer .gwt-TextBox");
		...
		if (c == 8 && playedMelody.length > 0) {
			...
			clearTimeout2(timerTimeCountry1);
			timerTimeCountry1 = setTimeout2("renderInput2();", 300);
			return;
		}

		clearTimeout2(timerTimeCountry1);
		timerTimeCountry1 = setTimeout("renderInput2();", 300);
		clearTimeout2(timerTimeCountry2);
		if (ngramField.val().length > 0 && (typeof something === "undefined")) {
			timerTimeCountry2 = setTimeout("submitQuery2();", 2000);
		}
	})
	contd.blur(renderInput2);                 
}

var clearSubmitTimeout2 = function () {
	clearTimeout2(timerTimeCountry2);
}

var submitQuery2 = function () {
	$('#sendButtonContainer .sendButton').click();
	prevNotePiano = null;
}
...
var renderMel2 = function(mel_str, canvas) {
...
var playedNote2 = function(pitch) {
	prevNotePiano = pitch;
	var prevNgram = $("#KeyboardTimeCountryContainer .gwt-TextBox").val();
	prevNgram = prevNgram.replace(/^\s+|\s+$/g, '');
	if (playedMelody.length > 0) {
		var prevNote = playedMelody[playedMelody.length - 1];
		diff = (0 - - pitch) - (0 - - prevNote);
		var newNgram = prevNgram + " " + diff;
		$("#KeyboardTimeCountryContainer .gwt-TextBox").val(newNgram);
	} else /* if (prevNgram != "") {
		var newNgram = prevNgram + ", " + pitch;
		$("#contentContainer .gwt-TextBox").val(newNgram);
	} else if (prevNgram == "") */ {
		$("#KeyboardTimeCountryContainer .gwt-TextBox").val(pitch);
	}
	renderInput2();
	playedMelody.push(pitch);
	noteTimes.push(new Date().getTime());
...
function addNoteAfter2(noteIndex) {
	var ints = getMelodyArray(0);
	var chord = "" + ints[noteIndex];
	var chordBase = chord;
	var duration = "";
	...
	selectedNoteIndex += 1;
	var new_mel_str = ints.join(" ");
	$("#KeyboardTimeCountryContainer .gwt-TextBox").val(new_mel_str);
	_gaq.push(['_trackEvent', "Search parameters", "Visual score editor: note copied"]);
}

function changeSelectedNote2(amount) {
	var ints = getMelodyArray(0);
	if (ints.length > selectedNoteIndex + amount) {
		selectedNoteIndex = selectedNoteIndex + amount;
	}
...
function durationIsDotted2(duration) {
	return $.inArray(duration, ["hd", "qd", "8d", "16d"]) != -1;
}

function changeDuration2(duration, amount) {
	var out = duration;
	for (var i = 0; i < durations.length; i++) {
		if (durations[i] == duration && i + amount < durations.length && i + amount >= 0) {
			out = durations[i - - amount];
...
function changeSelectedNoteDuration2(amount) {
	var ints = getMelodyArray2(0);
	var chord = "" + ints[selectedNoteIndex];
	var duration = "8";
	var baseChord = chord; 
	...
	var d2 = changeDuration2(duration, amount)
	chord = baseChord + "(" + d2 + ")";
	if (d2 == "q") chord = baseChord; 
	ints[selectedNoteIndex] = chord;
...
function transposeChord2(chord, amount) {
	var baseChord = chord;
	var duration = "";
	if (chord.indexOf("(") > 0 && chord.indexOf(")") > chord.indexOf("(")) {
		duration = chord.substring(chord.indexOf("(") + 1, chord.indexOf(")"));
...
function changeMelodyNote2(noteIndex, amount) {
	var ints = getMelodyArray2(0);

	ints[noteIndex] = transposeChord2(ints[noteIndex], amount);

	if (ints.length > noteIndex + 1) {
		ints[noteIndex + 1] = transposeChord2(ints[noteIndex + 1], -amount);
	} else {
		playedMelody2[playedMelody.length - 1] -= -amount;
	}
	var new_mel_str = ints.join(" ");
	$("#KeyboardTimeCountryContainer .gwt-TextBox").val(new_mel_str);
	_gaq.push(['_trackEvent', "Search parameters", "Visual score editor: note transposed"]);
...
function removeNote2(noteIndex) {
	var ints = getMelodyArray2(0);
	if (ints.length <= 1) return;
	...
	if (noteIndex < ints.length) {
		if (noteIndex > 0) {
			ints[noteIndex] = transposeChord2(ints[noteIndex], diff);
		} else {
			ints[0] = transposeChord2(ints[0], diff);
		}
	}
	var new_mel_str = ints.join(" ");
	$("#KeyboardTimeCountryContainer .gwt-TextBox").val(new_mel_str);
...
function scoreOnClick2(e) {
	if (getMelodyArray2(1) != null) {
		return;
	}
	...
	if (notes.length > 0) {
		...
		renderInput2();
		highlightNote2(selectedNoteIndex);
	} else {
		var new_mel_str = "57";
		playedMelody.push(57);
		$("#KeyboardTimeCountryContainer .gwt-TextBox").val(new_mel_str);
...
function highlightNote2(noteIndex) {
