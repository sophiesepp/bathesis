...
function language(prop) {
    ...
    window.history.pushState(null, null, "?lng=" + prop);
    var requestUrl = 'Languages/' + prop + '/main.lng';
    $.ajax({
        type: "GET",
        url: requestUrl,
        cache: false,
        ...
        success: function (data) {
            var mark = $('body').find('[class*=lng]');
            for (var i = 0; i < mark.length; i++) {
                var template = $(mark[i]);
                template.html($.tmpl(mark[i], data));
            }
            //console.log(template)
            /*
            var inputMark = $('body').find('input[data-lang]');
            
            for (var i = 0; i < inputMark.length; i++) {
            var inputTemplate = $(inputMark[i]);
                
            var inputClassName = inputTemplate.attr('data-value')
                
            $.each(data, function (index, value) {

            jQuery.map(value, function (obj, objValue) {
            if (objValue === inputClassName) {
            inputTemplate.val(obj);
            }
            });
            });
            }*/
    ...
    requestUrl = 'Languages/' + prop + '/querypanel.lng';
    try {
        $.ajax({
            type: "GET",
            url: requestUrl,
            ...
            success: function (data) {
                ...
                $('#ColumnsPanel').ColumnsPanel('refresh');
            },
            error: function () {
                EQ.core.texts = {};
                $('#QueryPanel').QueryPanel('refresh');
...
$(document).ready(function () {
    ...
    if (location.href.search(/lng=/) == -1) {
        window.history.pushState(null, null, "?lng=en");
    }

    language(location.search.slice(5));
